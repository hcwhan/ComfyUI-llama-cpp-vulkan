...
> 标注"支持"的 handler 在下拉框中有两个条目: 普通条目关闭思考, `-Thinking` 变体开启思考。GLM-4.1V 是纯思考模型, 无开关。Gemma4 E2B/E4B 无论开关均会思考 (仅 31B/26BA4B 真正支持开关), 思考内容都会从输出中剥离。下拉框按家族分组 (新版本在前), 完整列表以下拉框为准。
...
| MinerU2.5-Pro | 文档解析 (基于 Qwen2.5-VL) |
| Granite-Docling | 文档解析 |
| Generic-MTMD | 兜底 Handler (渲染模型内置 chat template) |

> 没有专用 Handler 的 VLM 可以选择 `Generic-MTMD` 兜底 Handler 以多模态方式运行; 纯文本 GGUF 模型无需 Handler, 用 `llm Model Loader` 以通用文本模式运行。

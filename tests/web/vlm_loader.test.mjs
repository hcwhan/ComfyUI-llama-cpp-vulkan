...
test("forced 与 none 档之间切换不重复缓存, toggle 档的用户值保留", () => {
    registerDef();
    // 初始 thinking=false 与 forced 档覆写值 true 相异, 使重复缓存可被识破:
    // 若切到 none 档时重复缓存, 缓存到的是 forced 档覆写值 true,
    // 切回 toggle 档恢复的就不再是用户值 false
    const node = createNode("VisionToggle", { thinking: false });
...
test("切回 toggle 档恢复后缓存即清除: 再改值后 toggle 档内再同步不被旧缓存覆写", () => {
    registerDef();
    const node = createNode("VisionToggle", { thinking: false });
    switchHandler(node, "VisionForced");
    switchHandler(node, "VisionToggle");
    // 切回 toggle 档已恢复 false 并清除缓存, 此后用户改为 true;
    // 若恢复处不 delete _userValue, 下面归一为 toggle 档的再同步
    // 会用残留缓存 false 覆写用户新值
    findWidget(node, "thinking").value = true;

...
- "@@@"  按节点模态替换(图像/视频/音频; text 节点 MEDIA_WORD 为空串,
         其可见预设不使用 @@@, 新增 text 预设时模态词直接写进模板);
         模板中占位符与相邻文字之间留空格以提升可读性 (邻接标点时不加),
         空格随替换保留, 不影响模型理解
...
user_prompt_presets = {
    ...
    "创意 - 提示词增强 (需custom_prompt)": {
        "use": ["text"],
        "content": '改写并增强下面的用户提示词, 用于文生图创作. 保留原意与关键词, 使其更具表现力, 视觉细节更丰富. **只输出改写后的提示词文本**, 不要输出推理过程或任何额外说明.\n\n用户提示词: "###"',
    },
    "创意 - 图片分析": {
        "use": ["image"],
